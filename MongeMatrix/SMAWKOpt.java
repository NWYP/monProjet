...
//	protected int [] setO2;
	...
	public SMAWKOpt(SlackMatrixLight matrix) {
	//	setO1 = matrix.getO1();
	//	setO2 = matrix.getO2();
		//access = 0;
		//total_op = 0;
	...
	public void smawk(Integer[] rows, Integer[] cols) throws InconsistentException {
		...
		if (rows.length == 1) {
			...
			min = 0;
			if(cols.length > 1) {
				min = valueAt2(rows[0], argmin);
				for (int  c : cols) {
					value = valueAt2(rows[0], c);
					if(min > value) {
						argmin = c;
						min = value;
					}
	...
	protected int getNegativePartialMG(Integer[] rows, Integer[] cols, int rowStart, int rowEnd, int colStart, int colEnd) throws InconsistentException {
		if(rowStart == rowEnd && colStart == colEnd) {
			return valueAt(rows[rowStart], cols[colStart]);
		}
		else if(rowStart == rowEnd || colStart == colEnd) {
			return findMinMG(Arrays.copyOfRange(rows, rowStart, rowEnd+1), 
					Arrays.copyOfRange(cols, colStart, colEnd+1));
		}
//		if(rowStart == rowEnd) 	{
//			for (int t1: Arrays.copyOfRange(rows, rowStart, rowEnd+1)){
//				System.out.print("last col "+colEnd + "\t");
//				for (int t2 : Arrays.copyOfRange(cols, colStart, colEnd+1)) {
//					System.out.print("S["+rowEnd+","+matrix.getCol(t2)+"] "+valueAt2(t1, t2) + "\t");
//					//System.out.print(valueAt2(t1, t2) + "\t");
//				}
//				System.out.print("\n");
//			}System.out.println("cc"+colStart+" "+colEnd+" "+rowStart+" "+rowStart);
//
////			if(colStart == colEnd) {
////				return valueAt(rows[rowStart], cols[colStart]);
////			}
////			else {
////				int value = valueAt(rows[rowStart], cols[colStart]);
////				for(int col = colStart+1; col < colEnd; col++) {
////					value = Math.min(value, valueAt(rowStart, col));
