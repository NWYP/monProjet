...
/**
 * Profile-based energetic overload checker for the Cumulative Scheduling Problem (CuSP).
 *
 * <p>This checker uses a profile-based energetic reasoning algorithm to detect whether a
 * set of tasks scheduled on a single resource of capacity {@code C} is infeasible.
 * The algorithm builds a resource-consumption profile from the tasks' time windows and
 * checks whether the accumulated energy demand can be satisfied within the available
 * resource capacity.</p>
 *
 * <p>Unlike {@link ProfileCheckerWithCriteria}, this variant tests <em>every</em>
 * candidate bound {@code b} (ECT and LCT values) without any pre-filtering criterion.</p>
 *
 * <p><b>Usage example:</b></p>
 * <pre>{@code
 * Task[] tasks = { new Task(0, 0, 10, 3, 2), new Task(1, 2, 8, 4, 3) };
 * ProfileChecker checker = new ProfileChecker(tasks, 5);
...
public class ProfileChecker {
    ...
    /**
     * Set of "delta" timepoints where overflow is detected during the global pass.
     * Used by sub-classes that apply a filtering criterion.
     */
    ...
    /**
     * Creates a new {@code ProfileChecker} for the given set of tasks and resource capacity.
     *
     ...
     * is done in {@code O(n log n)} and only once.</p>
     *
     ...
     */
    public ProfileChecker(Task[] tasks, int capacity) {
    ...
    /**
     * Runs the profile-based energetic overload check.
     *
     * <p>For every candidate bound {@code b} (union of all ECT and LCT values), the
     * method:</p>
     * <ol>
     *   <li>Decomposes tasks relative to {@code b} and builds the profile timeline.</li>
     *   <li>Simulates resource consumption along the timeline.</li>
     *   <li>Returns {@code false} as soon as an overload (ECT &gt; LCT) is detected.</li>
     * </ol>
     *
    ...
    public boolean overloadCheck() {
        ...
        for (Task task : tasks) {
            bounds.add(task.getLct());
            bounds.add(task.getEct());
        }
        for (int b : bounds) {
            int maxLct = buildDecomposedTimeline(b);
            if (maxLct != Integer.MAX_VALUE) {
                int ect = scheduleTasks(maxLct, false);
                if (ect > maxLct) {
                    return false;
                }
            }
     ...
     * before {@code b}, entirely after {@code b}, or straddle {@code b} in one of three
     * characteristic ways (left-overlap, right-overlap, spanning).</p>
     *
    ...
    private int buildDecomposedTimeline(int b) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // -- lst bucket (spanning tasks with ECT >= b) --
            ...
            // -- byEct bucket (tasks fully inside or left-overlapping [*, b]) --
    ...
    private int buildMergedTimeline(
        ...
        // Determine the first timepoint (earliest decomposed EST across the three lists)
     ...
     *
     * <p>This method initialises the profile over the full scheduling horizon
     ...
     */
    protected int buildGlobalTimeline() {
        profile.reset();
     ...
     *
     * <p>The simulation propagates any overflow (energy that could not be scheduled
     * in earlier intervals) forward in time. If the total overflow is positive at
     * the end, the method returns {@link Integer#MAX_VALUE} to signal an infeasible
     * configuration.</p>
     *
     * @param maxLct     the right boundary of the simulation (maximum LCT)
     * @param trackDelta if {@code true}, timepoints that have positive overflow are
     *                   added to {@link #delta} for use by the criteria-based variant
     * @return the ECT of the last non-idle interval, or {@link Integer#MAX_VALUE}
     *         if an overflow is detected
     */
    protected int scheduleTasks(int maxLct, boolean trackDelta) {
        ...
        while (t.time < maxLct) {
            ...
            // Split the slot if the overflow is consumed before the slot ends
    ...
    // -----------------------------------------------------------------------
    // Decomposed task accessors
    // -----------------------------------------------------------------------

    /**
     * Returns the decomposed earliest start time of task at 0-based index {@code taskId}
     * relative to bound {@code b}.
     *
     * <p>Note: the lists ({@code byEst1}, etc.) store values equal to
     * {@code task.getId() + 1}, which — because {@code ATask.getId()} returns
     * {@code id - 1} — equals the 0-based constructor argument {@code id}
     * (the direct array index into {@link #tasks}).</p>
     *
     * <p>The decomposition rule is:
     * <ul>
     *   <li>If {@code LCT ≤ b}: return original EST.</li>
     *   <li>If {@code LST < b} and {@code ECT < b}: return {@code EST + LCT - b}
     *       (middle case).</li>
     *   <li>If {@code LST < b} and {@code ECT ≥ b}: return LST (right-overlap).</li>
     *   <li>Otherwise: return {@code -1} (task does not contribute).</li>
     * </ul>
     * </p>
     *
     * @param taskId 0-based task index (direct index into {@link #tasks})
     * @param b      the right boundary
     * @return the decomposed EST, or {@code -1} if the task does not contribute
     */
    private int decomposedEst(int taskId, int b) {
        Task t = tasks[taskId];
        if (t.getLct() <= b) {
            return t.getEst();
        }
    ...
    /**
     * Returns the decomposed earliest completion time of task at index {@code taskId}
     * relative to bound {@code b}.
     *
     * @param taskId 0-based task index (stored as {@code getId() + 1} in the lists,
     *               where {@code getId()} returns the 0-based array index)
     * @param b      the right boundary
     * @return the decomposed ECT, or {@code -1} if the task does not contribute
     */
    private int decomposedEct(int taskId, int b) {
        Task t = tasks[taskId];
        if (t.getLct() <= b) {
            return t.getEct();
        }
    ...
    /**
     * Returns the decomposed latest completion time of task at index {@code taskId}
     * relative to bound {@code b}.
     *
     * @param taskId 0-based task index
     * @param b      the right boundary
     * @return the decomposed LCT, or {@code -1} if the task does not contribute
     */
    private int decomposedLct(int taskId, int b) {
        Task t = tasks[taskId];
        if (t.getLct() <= b) {
            return t.getLct();
        }
    ...
    /**
     * Returns the height (resource demand) of task at index {@code taskId}.
     * The bound {@code b} is currently unused but kept for API consistency.
     *
     * @param taskId 0-based task index
     * @param b      the right boundary (unused)
     * @return the task height
     */
    private int decomposedHeight(int taskId, int b) {
        return tasks[taskId].getH();
    }

    /**
     * Returns the decomposed processing time of task at index {@code taskId} relative
     * to bound {@code b}.
     *
     * @param taskId 0-based task index
     * @param b      the right boundary
     * @return the portion of the task's processing time that falls within {@code [*, b]}
     */
    private int decomposedP(int taskId, int b) {
        Task t = tasks[taskId];
    ...
    /**
     * Advances the current timepoint pointer to time {@code time}.
     *
     * <p>If the current timepoint already has that time, it is returned unchanged.
     * Otherwise a new {@link Timepoint} is inserted immediately after the current
     * one and the pointer is moved forward.</p>
     *
     ...
     *
     * @param values at least two integer values
