...
public class RelaxedKameugneChecker {
    Integer C;
    int n;
    Task[] tasks;
    private Profile tl;
    ...
    public RelaxedKameugneChecker(Task[] tasks, int C) {
        this.C = C;
        this.tasks = tasks;
        this.tl = new Profile();
        n = tasks.length;
    ...
    public boolean isConsistent()
    {
        ...
        for (Integer b : delta) {
            int peack = 0;
            for (int i = 0; i < n; i++) {
                if (decomposedTasksLct(i,b) == b)
                    peack += decomposedTasksHeight(i,b);
            }
            if (peack >= C) {
    ...
    private int decomposedTasksEnergy(int i, int b) {
        return decomposedTasksP(i,b) * decomposedTasksHeight(i,b);
    }
